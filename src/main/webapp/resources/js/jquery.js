		...
		(
				...
				function(window, noGlobal) {
					...
					(function(window) {
						 ...
						 * 
						 * @returns {Function(string, Object)} Returns the Object data after storing it on itself with property name the (space-suffixed) string and (if the cache is larger than
						...
						Expr = Sizzle.selectors = {
							...
							preFilter : {
								...
								"CHILD" : function(match) {
									/*
									 * matches from matchExpr["CHILD"] 1 type (only|nth|...) 2 what (child|of-type) 3 argument (even|odd|\d*|\d*n([+-]\d+)?|...) 4 xn-component of xn+y argument
									...
									return function(elem) {
										...
										return operator === "=" ? result === check : operator === "!=" ? result !== check : operator === "^=" ? check && result.indexOf(check) === 0
												: operator === "*=" ? check && result.indexOf(check) > -1 : operator === "$=" ? check && result.slice(-check.length) === check
														: operator === "~=" ? (" " + result.replace(rwhitespace, " ") + " ").indexOf(check) > -1 : operator === "|=" ? result === check
							...
							for (; i < len; i++) {
								if ((matcher = Expr.relative[tokens[i].type])) {
									...
									if (matcher[expando]) {
										...
										})).replace(rtrim, "$1"), matcher, i < j && matcherFromTokens(tokens.slice(i, j)), j < len && matcherFromTokens((tokens = tokens.slice(j))), j < len
												&& toSelector(tokens));
									}
						 ...
						 *            selector A selector or a pre-compiled selector function built with Sizzle.compile
						 * @param {Element}
						 *            context
						 * @param {Array}
						 *            [results]
					...
					jQuery.event = {
						...
						trigger : function(event, data, elem, onlyHandlers) {
							var handle, ontype, cur, bubbleType, special, tmp, i, eventPath = [ elem || document ], type = hasOwn.call(event, "type") ? event.type : event, namespaces = hasOwn.call(
									event, "namespace") ? event.namespace.split(".") : [];
					...
					jQuery.fn.extend({
						...
						html : function(value) {
							return access(this, function(value) {
								...
								if (typeof value === "string" && !rnoInnerhtml.test(value) && (support.htmlSerialize || !rnoshimcache.test(value))
					...
					jQuery.each([ "height", "width" ], function(i, name) {
						jQuery.cssHooks[name] = {
							...
							set : function(elem, value, extra) {
								...
								return setPositiveNumber(elem, value, extra ? augmentWidthOrHeight(elem, name, extra, support.boxSizing
					...
					function defaultPrefilter(elem, props, opts) {
						...
						var prop, value, toggle, tween, hooks, oldfire, display, checkDisplay, anim = this, orig = {}, style = elem.style, hidden = elem.nodeType && isHidden(elem), dataShow = jQuery
								._data(elem, "fxshow");
					...
					/*
					 * Prefilters 1) They are useful to introduce custom dataTypes (see ajax/jsonp.js for an example) 2) These are called: - BEFORE asking for a transport - AFTER param serialization
					 * (s.data is a string if s.processData is true) 3) key is the dataType 4) the catchall symbol "*" can be used 5) execution will start with transport dataType and THEN continue
					 * down to "*" if needed
					 */
					prefilters = {},
